...
war_of_legends_equipment.filter = function(unit_id, gear_id)    
	...
	if units[1] then
          ...
          result = "not found"
--          wesnoth.message("Filter_debugging4", string.format("war_of_legends_equipment.filter returns result= %s", result))
          return result
	end
      end
      if wesnoth.sides[units[1].side].controller == "ai" then
        result = "is ai"
--        wesnoth.message("Filter_debugging5", string.format("war_of_legends_equipment.filter returns result= %s", result))
      end
...
--            wesnoth.message("Filter_debugging_Iteration", string.format("war_of_legends_equipment.filter returns result= %s", gear_pos_temp))
...
--              wesnoth.message("Filter_debugging3", string.format("war_of_legends_equipment.filter returns result= %s", result))
              return result
            end
...
--              wesnoth.message("Filter_debugging2", string.format("war_of_legends_equipment.filter returns result= %s", result))
              return result
            end
          end
        end
      end
--  wesnoth.message("Filter_debugging1", string.format("war_of_legends_equipment.filter returns result= %s", result))
   return result
end

-- adds the gear id to a list that can be used by another unit on the recall list
war_of_legends_equipment.pool_add = function(gear_id) 
    local gear_number = 0
...
-- this function calls war_of_legends_equipment.filter and if the gear cannot be applied, we figure out what to do with it.
-- returns pass/fail
war_of_legends_equipment.unit = function(unit_id, gear_id)
    local output = ""
    local filter_result = war_of_legends_equipment.filter(unit_id, gear_id)
    ...
    elseif filter_result == "no room" or filter_result == "wrong type" then
      war_of_legends_equipment.pool_add(gear_id)
      output = "pass"
    else
      output = "error"
    end
--  wesnoth.message("unit_debugging1", string.format("war_of_legends_equipment.unit returns output= %s", output))
   return output
end

war_of_legends_equipment.remove = function(unit_id, gear_id)
      local result = ""
      local old_gear = ""
      local old_gear_id = ""
...
-- let's make sure this is really needed...  Yes, it is, but I'm not sure it's a bug with core [remove_object] etc.; I can't reproduce this in a simple test-case
	  ...
	  war_of_legends_helper.modify_unit({ id = unit_id }, { hitpoints = hack_u.hitpoints })
-- this does not work	  
...
war_of_legends_equipment.pool_remove = function(gear_id)
...
war_of_legends_equipment.item_drop = function(x_1, y_1, gear_id)
    local icon = ""
    for j in ipairs(equipment_list.the_list) do  
      if equipment_list.the_list[j].id == gear_id then
...
war_of_legends_equipment.item_take = function(x_1, y_1, gear_id)
    local result = "fail"
    local item_index = 0
    local id_temp = ""
    local x_temp = ""
...
return war_of_legends_equipment

...
function wesnoth.wml_actions.apply_gear(cfg)
        ...
        local result = war_of_legends_equipment.unit(unit_id, gear_id)
...
--            local take_result = war_of_legends_equipment.item_take(eq_unit[1].x, eq_unit[1].y, gear_id)
--            if take_result == "pass" then
            if eq_unit[1] then
              war_of_legends_equipment.item_take(eq_unit[1].x, eq_unit[1].y, gear_id)
...
function wesnoth.wml_actions.remove_gear(cfg)
        ...
        local result = war_of_legends_equipment.remove(unit_id, gear_id)
        if result == "on_map" then
          local eq_unit = wesnoth.units.find_on_map({ id = unit_id })
          war_of_legends_equipment.item_drop(eq_unit[1].x, eq_unit[1].y, gear_id)
--          wesnoth.message(string.format("%s on map", unit_id))
...
function wesnoth.wml_actions.gear_item(cfg)
        ...
        war_of_legends_equipment.item_drop(x_1, y_1, gear_id)
end     
...
function wesnoth.wml_actions.remove_gear_item(cfg)
        ...
        war_of_legends_equipment.item_take(x_1, y_1, gear_id)
end     
